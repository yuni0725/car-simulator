import numpy as np
from simulations.geometry import Point, Rectangle, Circle, Ring
...
class Entity:
    ...
    def set_control(self, inputSteering: float, inputAcceleration: float):
        self.inputSteering = np.deg2rad(inputSteering * 2)
    ...
    def tick(self, dt: float):
        if self.movable:
            ...
            new_center = (
                self.center
                + (speed + new_speed) * Point(np.cos(angle), np.sin(angle)) * dt / 2.0
            )
            new_velocity = Point(
