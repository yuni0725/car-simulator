...
from simulations.agents import Car, RingBuilding, CircleBuilding, Painting, Pedestrian
...
cb = CircleBuilding(
    Point(world_width / 2, world_height / 2), inner_building_radius, "gray80"
)
w.add(cb)
rb = RingBuilding(
    Point(world_width / 2, world_height / 2),
    inner_building_radius
    + num_lanes * lane_width
    + (num_lanes - 1) * lane_marker_width,
...
for lane_no in range(num_lanes - 1):
    lane_markers_radius = (
        inner_building_radius
        + (lane_no + 1) * lane_width
        + (lane_no + 0.5) * lane_marker_width
    )
    lane_marker_height = np.sqrt(
        2
        * (lane_markers_radius**2)
        * (1 - np.cos((2 * np.pi) / (2 * num_of_lane_markers)))
...
c1.max_speed = 10.0  # let's say the maximum is 30 m/s (108 km/h)
...
if not human_controller:
    ...
    for k in range(600):
        ...
        if c1.distanceTo(cb) < desired_lane * (lane_width + lane_marker_width) + 0.2:
            ...
            c1.distanceTo(rb)
            < (num_lanes - desired_lane - 1) * (lane_width + lane_marker_width) + 0.3
        ):
            lp += 1.0
    ...
    from simulations.interactive_controllers import KeyboardController

    c1.set_control(0.0, 0.0)  # Initially, the car will have 0 steering and 0 throttle.
