...
class Network(nn.Module):
    def __init__(self, input_size, nb_action):
        ...
        # ✅ 여러 층으로 점진적 차원 감소
        self.conv1 = nn.Conv1d(1, 16, kernel_size=5)  # 90 → 86
        self.conv2 = nn.Conv1d(16, 32, kernel_size=7)  # 86 → 80
        self.conv3 = nn.Conv1d(32, 16, kernel_size=9)  # 80 → 72
        self.conv4 = nn.Conv1d(16, 1, kernel_size=41)  # 72 → 32

        self.fc_other = nn.Linear(input_size - 90, 32)

        # 최종 크기: 32(conv) + 32(other) = 64
    ...
    def forward(self, state):
        ...
        # ✅ 점진적 특징 추출
        x1 = lidar_data.unsqueeze(1)  # [1, 1, 90]
        x1 = F.relu(self.conv1(x1))  # [1, 16, 86]
        x1 = F.relu(self.conv2(x1))  # [1, 32, 80]
        x1 = F.relu(self.conv3(x1))  # [1, 16, 72]
        x1 = F.relu(self.conv4(x1))  # [1, 1, 32]
        x1 = x1.squeeze(1)  # [1, 32]
